...
def text2hex(pt):
    ...
    for i in pt:
        ascii_val = int(ord(i))
        first_hex = int(ascii_val / 16)
        second_hex = ascii_val % 16
        if second_hex > 9:
            letters = "FEDCBA"
            diff = 15 - second_hex
            second_hex = letters[diff]
...
def make_blocks(converted_pt):
    ...
    if 16 >= len(converted_pt) > 0:
        ...
        blocks.append(converted_pt)

    # if hexadecimal conversion is more than 16 characters, break into 16 character blocks
    elif len(converted_pt) > 16:
        temp = converted_pt
        while len(temp) > 16:
            block = temp[0:16]
            blocks.append(block)
            temp = temp[16:len(temp) + 1]
        if 16 >= len(temp) > 0:
            while len(temp) < 16:
                temp = temp + "20"
            blocks.append(temp)
